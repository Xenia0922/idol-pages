  ...
  return (
    <div className="w-full">
      ...
      <select
        ...
        disabled={evLoading}
        className={`${selCls} max-w-xs mx-auto block mb-4 disabled:opacity-50`}
      >
        <option value="">
      ...
      <div className="frost-card p-6 text-center">
        ...
        {items.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mt-4">
            {items.map((it) => (
              <div
                key={it.id}
                className="relative aspect-square rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-white/10 group"
              >
                <img
                  src={it.preview}
                  alt=""
        ...
        >
          {uploading
            ? "上传中..."
            : `上传 ${items.length > 0 ? items.length + " 张" : ""}`.trim()}
        </button>

        {msg && (
          <p
            className={`mt-3 text-sm ${msg.startsWith("❌") ? "text-red-400" : msg.startsWith("⚠️") ? "text-amber-500" : "text-green-500"}`}
          >
            {msg}
          </p>
        )}
        <p className="text-[10px] text-gray-400 mt-2">
          粉丝上传的照片需管理员审核后才会公开展示
        </p>
      </div>
    </div>
  );
