...
export default function AdminPanel() {
  const [code, setCode] = useState(() => {
    if (typeof window !== 'undefined') return localStorage.getItem('fansite-admin') || '';
    return '';
  });
  ...
  useEffect(() => {
    ...
    inited.current = true;
    const stored = (typeof window !== 'undefined') ? (localStorage.getItem('fansite-admin') || '') : '';
    if (!stored) return;
    (async () => {
      try {
  ...
  const logout = () => {
    try { localStorage.removeItem('fansite-admin'); } catch (e) {}
    setCode('');
    setAuthed(false);
  };
