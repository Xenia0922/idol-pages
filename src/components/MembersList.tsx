...
export default function MembersList({ initial }: { initial: Member[] }) {
  ...
  if (!loading && members.length === 0)
    return (
      <p className="text-center text-gray-400 py-16 content-enter">暂无成员</p>
    );
  ...
  return (
    ...
    >
      <div>
        <p className="text-xs font-bold text-pink-500 tracking-widest text-center mb-5">
          正在活动
        </p>
        ...
        {graduated.length > 0 && (
          <div className="border-t border-gray-100 dark:border-gray-800 pt-10">
            <p className="text-xs font-bold text-gray-400 tracking-widest text-center mb-5">
              已毕业
            </p>
            <div className="grid grid-cols-3 gap-4 sm:gap-6">
              {graduated.map((m, i) => (
                <Card key={m.id} m={m} index={i + active.length} />
              ))}
            </div>
          </div>
        )}
