...
export default function AdminGalleryEdit({ code }: { code: string }) {
  ...
  const badgeOf = (m: string) =>
    groups.find((g) => g.key === m) || groups[groups.length - 1];
  ...
  const getMember = (id: string) =>
    photosRef.current.find((p) => p.id === id)?.member;
  ...
  const persistOrder = useCallback(
    async (arr: Photo[]) => {
      ...
      const groupPayload = groups.map((g) => ({
        member: g.key,
        ids: arr
          .filter((p) => !fids.includes(p.id) && p.member === g.key)
          .map((p) => p.id),
      }));
      // 广场精选
      groupPayload.push({
  ...
  return (
    <div className="frost-card p-5">
                    ...
                    <div
                      ...
                      onDragOver={(e) => {
                        ...
                        if (dragId && dragId !== p.id)
                          applyReorder(getMember(dragId) || "", dragId, p.id);
                      }}
                      onDrop={(e) => {
                        e.preventDefault();
                        setDragId(null);
                      ...
                      onClick={() =>
                        setLightbox({
                          images: g.items.map((x) => ({ src: x.url })),
                          idx: g.items.indexOf(p),
                        })
                      }
                ...
                {featured.map((p) => (
                  <div
                    ...
                    onDragOver={(e) => {
                      e.preventDefault();
                      if (dragId && dragId !== p.id)
                        applyFeaturedReorder(dragId, p.id);
                    }}
                    onDrop={(e) => {
                      e.preventDefault();
                      setDragId(null);
                    ...
                    onClick={() =>
                      setLightbox({
                        images: featured.map((x) => ({ src: x.url })),
                        idx: featured.indexOf(p),
                      })
                    }
      ...
      {saving && (
        <p className="text-xs text-gray-400 mt-3 text-center">排序已保存</p>
      )}
      ...
      {lightbox && (
        <ImageLightboxOverlay
          ...
          onPrev={() =>
            setLightbox((l) =>
              l
                ? { ...l, idx: (l.idx - 1 + l.images.length) % l.images.length }
                : l,
            )
          }
          onNext={() =>
            setLightbox((l) =>
              l ? { ...l, idx: (l.idx + 1) % l.images.length } : l,
            )
          }
        />
      )}
