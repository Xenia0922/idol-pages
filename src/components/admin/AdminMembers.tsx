...
export default function AdminMembers({ code }: { code: string }) {
  ...
  const save = async () => {
    ...
    const body = {
      ...
      sort_order:
        form.sort_order != null && Number.isFinite(+form.sort_order)
          ? +form.sort_order
          : 99,
    };
    try {
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <Field label="画廊图片（九宫格，逐个上传）">
          <MemberGalleryUpload
            ...
            value={form.gallery || []}
            onChange={(updater) =>
              setForm((f) => ({ ...f, gallery: updater(f.gallery || []) }))
            }
          />
        </Field>
        <div className="grid grid-cols-2 gap-3">
