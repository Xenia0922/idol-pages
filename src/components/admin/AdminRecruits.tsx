...
export default function AdminRecruits({ code }: Props) {
  ...
  const AdCard = ({ dark }: { dark: boolean }) => {
    ...
    const cardStyle: CSSProperties = dark
      ? {
          background:
            "linear-gradient(160deg, rgba(255,255,255,0.07), rgba(255,255,255,0.035))",
      ...
      : {
          background:
            "linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.52))",
    ...
    return (
      <div style={cardStyle} className="rounded-2xl p-4">
        ...
        <p
          style={{ color: dark ? "#a59fc0" : "var(--text-soft)" }}
      ...
      <div className="frost-card p-5">
        ...
        <div className="space-y-3">
          <input
            ...
            placeholder="标题（如：研修生招募）"
            className={INPUT_CLS}
          />
          <input
            value={form.subtitle}
            onChange={(e) => setForm({ ...form, subtitle: e.target.value })}
            placeholder="副标题（如：你的团体简介标语）"
            className={INPUT_CLS}
          />
          <textarea
            value={form.body}
          ...
          <input
            ...
            placeholder="按钮文案（可留空，默认「查看详情 →」）"
            className={INPUT_CLS}
          />
          <input
            value={form.cta_url}
            onChange={(e) => setForm({ ...form, cta_url: e.target.value })}
            placeholder="跳转链接（需 http(s):// 开头）"
            className={INPUT_CLS}
          />
