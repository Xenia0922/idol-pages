...
export default function UpcomingCountdown({
  ...
  useEffect(() => {
    ...
    const localSources = [ssr?.events, initial].filter(
      (s): s is EventRow[] => Array.isArray(s) && s.length > 0,
    );
  ...
  if (!loading && empty)
    return (
      <p className="text-center text-gray-400 text-sm py-4 content-enter">
        暂无即将到来的演出
      </p>
    );
  ...
  return (
    <SkeletonSwap
      ...
      skeleton={
        <div
          className="frost-card p-4 text-center max-w-sm mx-auto"
          aria-hidden="true"
        >
    ...
    >
      {event && cd && (
        <div className="frost-card p-4 text-center max-w-sm mx-auto">
          ...
          {event.venue && (
            <p className="text-[11px] text-gray-400 mb-1.5">{event.venue}</p>
          )}
          <p className="text-xl font-black text-[var(--accent)] tabular-nums font-mono">
            {text}
          </p>
        </div>
      )}
