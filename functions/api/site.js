...
async function ensureTable(env) {
  ...
  try {
    const { results } = await env.DB.prepare(
      "SELECT COUNT(*) AS c FROM site_config",
    ).all();
    ...
    console.error("[site] seed failed:", e.message);
  }
}
...
async function loadConfig(env) {
  ...
  for (const r of results) {
    cfg[r.key] = JSON_KEYS.has(r.key)
      ? safeParse(r.value, r.key === "hero_config" ? {} : [])
      : r.value;
...
export async function onRequest(context) {
  ...
  try {
    ...
    console.error("[site] ensureTable error:", e.message);
  }

  if (request.method === "GET")
    return withTable(env, ensureTable, async () => json(await loadConfig(env)));
  if (request.method === "PUT")
    return withTable(env, ensureTable, () => updateConfig(request, env));
