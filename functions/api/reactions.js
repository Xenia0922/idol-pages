...
async function getReactions(request, env) {
  ...
  if (idsParam) {
    ...
    const { results } = await env.DB.prepare(
      `SELECT target_id, emoji, COUNT(*) as count FROM reactions WHERE target_type = ? AND target_id IN (${placeholders}) GROUP BY target_id, emoji`,
    )
      .bind(type, ...ids)
      .all();
    const { results: mine } = await env.DB.prepare(
      `SELECT target_id, emoji FROM reactions WHERE target_type = ? AND target_id IN (${placeholders}) AND ip = ?`,
    )
      .bind(type, ...ids, ip)
      .all();
    const map = {};
...
async function toggleReaction(request, env) {
  try {
    ...
    if (!ALLOWED_EMOJIS.includes(emoji))
      return json({ error: "emoji 不在白名单" }, 400);

    const ip = request.headers.get("cf-connecting-ip") || "unknown";

    // toggle：已存在则删除，不存在则插入
    const existing = await env.DB.prepare(
      "SELECT id FROM reactions WHERE target_type = ? AND target_id = ? AND emoji = ? AND ip = ?",
    )
    ...
    if (existing) {
      await env.DB.prepare(
        "DELETE FROM reactions WHERE target_type = ? AND target_id = ? AND emoji = ? AND ip = ?",
      )
        .bind(type, id, emoji, ip)
        .run();
      // 返回新的 count
      const { results } = await env.DB.prepare(
        "SELECT COUNT(*) as count FROM reactions WHERE target_type = ? AND target_id = ? AND emoji = ?",
      )
        .bind(type, id, emoji)
        .all();
      return json({
      ...
      await env.DB.prepare(
        "INSERT INTO reactions (target_type, target_id, emoji, ip, created_at) VALUES (?, ?, ?, ?, ?)",
      )
