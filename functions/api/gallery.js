...
async function seedIfEmpty(env) {
  const cnt = await env.DB.prepare(
    "SELECT COUNT(*) c FROM gallery_photos",
  ).first();
  if (cnt && cnt.c > 0) return;
  const seeded = await env.DB.prepare(
    "SELECT value FROM gallery_meta WHERE key='seeded'",
  ).first();
  if (seeded) return;
  ...
  try {
    ...
    const { results: eres } = await env.DB.prepare(
      "SELECT id,url FROM gallery_extras",
    ).all();
    for (const e of eres || []) {
      if (e.url && e.url.trim()) {
        rows.push({
  ...
  if (rows.length) {
    ...
    const batch = rows.map((r) =>
      stmt.bind(r.id, r.url, r.member, r.sort, r.created_at),
    );
    await env.DB.batch(batch);
  }
  await env.DB.prepare(
    "INSERT OR IGNORE INTO gallery_meta (key,value) VALUES ('seeded','1')",
  ).run();
}
...
export async function onRequest(context) {
  ...
  if (request.method === "GET") {
    try {
      ...
      return json(
        { photos: [], featured: [], isAdmin: false, error: e.message },
        500,
      );
    }
  }
  ...
  if (request.method === "POST") {
    ...
    try {
      ...
      if (!/^\/api\/photos|^https?:\/\//.test(url)) {
        return json(
          { error: "仅支持本站图片（/api/photos?...）或 http(s) 链接" },
          400,
        );
      }
      const max = await env.DB.prepare(
        "SELECT COALESCE(MAX(sort),0) m FROM gallery_photos",
      ).first();
  ...
  if (request.method === "PUT") {
    ...
    try {
      ...
      try {
        await env.DB.prepare(
          "UPDATE gallery_photos SET featured = ? WHERE id = ?",
        )
          .bind(featured, id)
          .run();
      } catch (colErr) {
    ...
    try {
      ...
      if (Array.isArray(b.groups)) {
        ...
        for (const grp of b.groups) {
          ...
          ids.forEach((id, wi) =>
            assignments.push({ id, sort: gi * 1000000 + wi }),
          );
          gi++;
        }
      ...
      const stmt = env.DB.prepare(
        "UPDATE gallery_photos SET sort = ? WHERE id = ?",
      );
  ...
  if (request.method === "DELETE") {
    ...
    try {
      ...
      if (!id) return json({ error: "缺少 id" }, 400);
      await env.DB.prepare("DELETE FROM gallery_photos WHERE id = ?")
        .bind(id)
        .run();
      return json({ ok: true });
    } catch (e) {
