...
async function ensureTable(env) {
  ...
  try {
    await env.DB.prepare("ALTER TABLE recruits ADD COLUMN subtitle TEXT").run();
  } catch (e) {
    /* 已存在则忽略 */
  }
  try {
    ...
    console.error("[recruits] seed failed:", e.message);
  }
}
...
export async function onRequest(context) {
  ...
  try {
    ...
    console.error("[recruits] ensureTable error:", e.message);
  }
  if (request.method === "GET")
    return withTable(env, ensureTable, () => listRecruits(request, env));
  if (request.method === "POST")
    return withTable(env, ensureTable, () => createRecruit(request, env));
  if (request.method === "PUT")
    return withTable(env, ensureTable, () => putRecruit(request, env));
  if (request.method === "DELETE")
    return withTable(env, ensureTable, () => deleteRecruit(request, env));
...
async function listRecruits(request, env) {
  ...
  if (all) {
    ...
    const { results } = await env.DB.prepare(
      "SELECT * FROM recruits ORDER BY sort_order ASC, id DESC",
    ).all();
    return json(results);
  }
  ...
  try {
    const { results: all } = await env.DB.prepare(
      "SELECT id, deadline FROM recruits WHERE enabled = 1",
    ).all();
    const now = Date.now();
...
async function createRecruit(request, env) {
  ...
  try {
    ...
    const body = String(b.body || "")
      .trim()
      .slice(0, 200);
    const cta_text = String(b.cta_text || "查看详情 →")
      .trim()
      .slice(0, 40);
    const cta_url = String(b.cta_url || "")
      .trim()
      .slice(0, 500);
    ...
    if (!sort_order || sort_order < 0) {
      const { results } = await env.DB.prepare(
        "SELECT MAX(sort_order) AS m FROM recruits",
      ).all();
      const maxSo = results[0] && results[0].m != null ? results[0].m : 0;
      sort_order = maxSo + 1;
    }

    if (!title || !body || !cta_url)
      return json({ error: "标题 / 正文 / 链接必填" }, 400);
...
async function reorderRecruits(order, env) {
  for (const item of order) {
    ...
    if (id) {
      await env.DB.prepare("UPDATE recruits SET sort_order = ? WHERE id = ?")
        .bind(so, id)
        .run();
    }
  }
...
async function updateRecruit(b, env) {
  try {
    ...
    if (b.title !== undefined) {
      sets.push("title = ?");
      binds.push(String(b.title).trim().slice(0, 60));
    }
    if (b.subtitle !== undefined) {
      sets.push("subtitle = ?");
      binds.push(String(b.subtitle).trim().slice(0, 60));
    }
    if (b.body !== undefined) {
      sets.push("body = ?");
      binds.push(String(b.body).trim().slice(0, 200));
    }
    if (b.cta_text !== undefined) {
      sets.push("cta_text = ?");
      binds.push(String(b.cta_text).trim().slice(0, 40));
    }
    ...
    if (b.deadline !== undefined) {
      sets.push("deadline = ?");
      binds.push(b.deadline ? String(b.deadline).slice(0, 16) : null);
    }
    if (b.enabled !== undefined) {
      sets.push("enabled = ?");
      binds.push(b.enabled === false || b.enabled === 0 ? 0 : 1);
    }
    ...
    binds.push(id);
    await env.DB.prepare(`UPDATE recruits SET ${sets.join(", ")} WHERE id = ?`)
      .bind(...binds)
      .run();
    return json({ ok: true });
  } catch (e) {
