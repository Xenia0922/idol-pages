...
async function uploadPhoto(request, env) {
  try {
    ...
    if (files.length > MAX_FILES)
      return json({ error: `一次最多上传 ${MAX_FILES} 张` }, 400);
    ...
    for (const f of files) {
      ...
      if (f.size > MAX_SIZE)
        return json({ error: "单张图片不能超过 23MB" }, 400);
    }

    // 限流（粉丝）：5 秒内本 IP 上传图片总数不超过 MAX_FILES
    const ip = request.headers.get("cf-connecting-ip") || "unknown";
    if (!isAdmin) {
      const allowed = await rateAllow(
        env,
        ip,
        "photo",
        MAX_FILES,
    ...
    for (const file of files) {
      ...
      const key = isAdmin
        ? `uploads/${member}/${id}.${ext}`
        : `uploads/pending/${member}/${id}.${ext}`;
