...
export async function onRequest(context) {
  ...
  if (request.method === "GET")
    return withTable(env, ensureEvents, () => listEvents(request, env));
  if (request.method === "POST")
    return withTable(env, ensureEvents, () => createEvent(request, env));
  if (request.method === "PUT")
    return withTable(env, ensureEvents, () => putEvent(request, env));
  if (request.method === "DELETE")
    return withTable(env, ensureEvents, () => deleteEvent(request, env));
...
async function putEvent(request, env) {
  ...
  try {
    ...
    if (b.performers !== undefined) {
      ...
      binds.push(
        JSON.stringify(Array.isArray(b.performers) ? b.performers : []),
      );
    }
